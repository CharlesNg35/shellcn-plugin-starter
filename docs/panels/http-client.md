...
The execute route receives the request method, URL/path, headers, and body from
the panel. Validate allowed hosts and paths server-side. Do not let the panel
become an arbitrary SSRF primitive; route through the connection's configured
target and `cfg.Net`.
