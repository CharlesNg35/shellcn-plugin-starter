...
A template for building an **external (out-of-tree) protocol plugin** for
...
don't fork the gateway — you write a small Go program against the **plugin SDK**,
...
audit, and policy as a built-in. No gateway rebuild, no frontend code.
...
Clone this repo and build — the plugin SDK is pulled from the Go proxy like any
other dependency (no need to clone the gateway):

```sh
git clone https://github.com/CharlesNg35/shellcn-plugin-starter.git
cd shellcn-plugin-starter
